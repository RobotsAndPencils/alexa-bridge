...
function sendReply(replyToId) {
  ...
  var res = res_next[0];
  var next = res_next[1];
  var alexaRequest = res_next[2];
  var reply = res_next[3];
...
function createAlexaReply(activity) {
  ...
  if (process.env.closeSessionPhrase && process.env.closeSessionPhrase === (activity.speak || activity.text)) {
    console.log('closeSessionPhrase detected')
    shouldEndSession = true;
  } else if (process.env.leaveSessionOpen == 'false') {
    shouldEndSession = true;
  }
  var reply = {
    "version": "1.0",
  ...
  if (process.env.useHeroCardAttachmentAsAlexaCard
    ...
    && activity.attachments[0].contentType == "application/vnd.microsoft.card.hero") {
      ...
      reply.response.directives = [
      {
        ...
        "template": {
          "type": "BodyTemplate7",
          "token": activity.replyToId,
          "title": activity.attachments[0].content.title,
          "backButton": "HIDDEN",
          "image": {
            "sources": [activity.attachments[0].content.images[0].url]
          }
        }
      }
    ];
...
function createAlexaProgressReply(requestId, msg) {
  if (msg || process.env.progressiveResponse) {
    var reply = {
      "header": {
        "requestId": requestId
      },
      "directive": {
        ...
        "speech": msg || process.env.progressiveResponse
      }
    }
    return reply;
  }
...
function botSays(activity) {
  ...
  if (process.env.useMultipleResponses != 'false' && activity.inputHint == 'ignoringInput' && activity.replyToId in responses) {
    var channelData = responses[activity.replyToId][2].channelData;
    var reply = createAlexaProgressReply(channelData.alexa_requestId, activity.text);
    sendDirective(reply, channelData.alexa_apiAccessToken, channelData.alexa_apiEndpoint);
    return;
  }
...
async function sendDirective(msg, apiAccessToken, apiEndpoint) {
  var headers = {
    'Authorization': "Bearer " + apiAccessToken
  };
  console.log('Sending directive')
  console.log(util.inspect({msg, apiAccessToken, apiEndpoint}, false, null));
  await rp({ method: 'post', url: apiEndpoint + '/v1/directives', headers: headers, json: msg })
    .then((body) => {
  ...
  bot.postActivity(activity)
    .subscribe(id => {
      if (id != 'retry') {
        ...
        responses[id] = [res, next, activity];

        if (activity.channelData) {
          var busyReply = createAlexaProgressReply(channelData.alexa_requestId);
          if (busyReply) {
            sendDirective(busyReply, activity.channelData.alexa_apiAccessToken, activity.channelData.alexa_apiEndpoint);
          }
        }
      }
    }, error => {
